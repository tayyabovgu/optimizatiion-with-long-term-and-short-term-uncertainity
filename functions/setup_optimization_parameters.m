function [cost, Grid, Bat, heat, CHP, opt, ANZ, NET, mpc, NOD, PROF, LIN, H2] = setup_optimization_parameters(opt, path, SetC, Scenario, EV)
    ...
    Grid.V      = sdpvar(ANZ.K,opt.Horizon); % v_n (Voltage squared)
    Grid.L      = sdpvar(ANZ.L,opt.Horizon); % l_n (Current squared)
    Grid.P_br   = sdpvar(ANZ.L, opt.Horizon); % P_s (Branch Active Power)
    Grid.Q_br   = sdpvar(ANZ.L, opt.Horizon); % Q_s (Branch Reactive Power)
    Grid.R_Line      = (LIN.l(1:ANZ.L).*LIN.r(1:ANZ.L)); % Ohms
    Grid.X_Line      = (LIN.l(1:ANZ.L).*LIN.x(1:ANZ.L)); % Ohms
    Grid.P_in=sdpvar(ANZ.K,opt.Horizon); % P_in
    Grid.Q_in=sdpvar(ANZ.K,opt.Horizon); % Q_in
    Grid.Pshed= sdpvar(1,opt.Horizon); % Grid Active Power Import
    Grid.Qshed= sdpvar(1,opt.Horizon); % Grid Reactive Power Import
    ...
    cost.HSyr=10; 
    % *** H2 'bat_loss' FIX ***
    % Create H2 struct with its own loss/decay parameters
    H2.loss = 0.02; % Based on your original Grid.echh2 = 0.98
    H2.decay = 0.01; % From your original Grid.selfdchh
    ...
    CHP.n_CHP=0; % Set to 0 to disable
    ...
    if opt.Horizon == 24
        ...
        % This is the fix for the *original* 8760-hour error
