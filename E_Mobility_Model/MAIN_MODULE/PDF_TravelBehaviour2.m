function PDF_TravelBehaviour2()
    ...
    subplot(2,2,4);
    t_start_am = Fitness_Travel('Trip starting time(AM)', 'Normal');
...
function freq = Fitness_Travel(sheet, fit_method)
    % load data
    [num_freq,~,~] = xlsread('travel_behaviour.xlsx', sheet);
    statistics.x = num_freq(:, 1);
    statistics.y = num_freq(:, 2) / sum(num_freq(:, 2));    % normalize
    % -- http://cn.mathworks.com/help/stats/fitdist.html?refresh=true
    bins = statistics.x;
