function [waiting_time,EV_public_load,EVCS_state,EV_state] = GetEVPublicLoad(EV_Profile,Horizon,nEVs,nEVCS)
%load('EV_data.mat');
...
% if EVstate = -1 then it is charging
% if EVstate = 0 then it is driving
...
EV_public_load(1,i) = 22;
end
end
% disp(sum(waiting_time))
% dum = length(EV_state(5,find(EV_state(5,:)==1)));
% disp(dum)
end